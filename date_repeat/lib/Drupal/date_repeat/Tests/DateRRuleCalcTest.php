...
class DateRRuleCalcTest extends WebTestBase {
   ...
   */
  public static $modules = array('date_api', 'date_repeat');
  ...
  public function testDateRRuleCalc() {
    ...
    $shouldbe = '';
    $result = implode(', ', $dates->compute());
    $this->assertEqual($result, $shouldbe, $rule . '; results: ' . $result);
    ...
    $result = implode(', ', (array) $dates->compute());
    $this->assertEqual($result, $shouldbe, $rule . '; results: ' . $result);
